...
if ! sysctl net.ipv4.tcp_available_congestion_control | grep -q "ebpf_cubic"; then
    echo "ebpf_cubic not registered - nothing to clean up"
    exit 0
fi
...
if sysctl net.ipv4.tcp_available_congestion_control | grep -q "ebpf_cubic"; then
    echo "ebpf_cubic is still registered after daemon shutdown"
    echo "This can happen if the struct_ops link was not properly detached"
    echo ""
    echo "To fix this, you may need to:"
    echo "  1. Unload any manually pinned BPF objects: rm -f /sys/fs/bpf/ebpf_cubic*"
